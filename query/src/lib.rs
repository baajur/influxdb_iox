...
pub trait PartitionChunk: Debug + Send + Sync {
    ...
    /// Idea is to make this return a list of Strings in the case
    /// where the result can be answered from metadata only, and the
    /// query layer will fall back to the general case if that is not possible.
    ///
